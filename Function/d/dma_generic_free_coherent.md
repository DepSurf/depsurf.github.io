# Function: <code>dma_generic_free_coherent</code>

## Status
<b>Regular</b>
<ul>
<li>
<details>
<summary>In <code>4.4.0-21-generic-amd64</code>: ✅</summary>

```c
void dma_generic_free_coherent(struct device * dev, size_t size, void * vaddr, dma_addr_t dma_addr, struct dma_attrs * attrs)
```

```json
{
  "name": "dma_generic_free_coherent",
  "collision_type": "Unique Global",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071579062064,
      "name": "dma_generic_free_coherent",
      "external": true,
      "loc": "arch/x86/kernel/pci-dma.c:122",
      "file": "arch/x86/kernel/pci-dma.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "arch/x86/kernel/pci-swiotlb.c:x86_swiotlb_free_coherent",
        "arch/x86/kernel/amd_gart_64.c:gart_free_coherent"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071579062064,
      "name": "dma_generic_free_coherent",
      "section": ".text",
      "bind": "STB_GLOBAL",
      "size": 39
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>4.8.0-22-generic-amd64</code>: ✅</summary>

```c
void dma_generic_free_coherent(struct device * dev, size_t size, void * vaddr, dma_addr_t dma_addr, long unsigned int attrs)
```

```json
{
  "name": "dma_generic_free_coherent",
  "collision_type": "Unique Global",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071579058464,
      "name": "dma_generic_free_coherent",
      "external": true,
      "loc": "arch/x86/kernel/pci-dma.c:122",
      "file": "arch/x86/kernel/pci-dma.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "arch/x86/kernel/pci-swiotlb.c:x86_swiotlb_free_coherent",
        "arch/x86/kernel/amd_gart_64.c:gart_free_coherent"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071579058464,
      "name": "dma_generic_free_coherent",
      "section": ".text",
      "bind": "STB_GLOBAL",
      "size": 39
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>4.10.0-19-generic-amd64</code>: ✅</summary>

```c
void dma_generic_free_coherent(struct device * dev, size_t size, void * vaddr, dma_addr_t dma_addr, long unsigned int attrs)
```

```json
{
  "name": "dma_generic_free_coherent",
  "collision_type": "Unique Global",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071579057504,
      "name": "dma_generic_free_coherent",
      "external": true,
      "loc": "arch/x86/kernel/pci-dma.c:122",
      "file": "arch/x86/kernel/pci-dma.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "arch/x86/kernel/pci-swiotlb.c:x86_swiotlb_free_coherent",
        "arch/x86/kernel/amd_gart_64.c:gart_free_coherent"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071579057504,
      "name": "dma_generic_free_coherent",
      "section": ".text",
      "bind": "STB_GLOBAL",
      "size": 39
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>4.13.0-16-generic-amd64</code>: ✅</summary>

```c
void dma_generic_free_coherent(struct device * dev, size_t size, void * vaddr, dma_addr_t dma_addr, long unsigned int attrs)
```

```json
{
  "name": "dma_generic_free_coherent",
  "collision_type": "Unique Global",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071579049504,
      "name": "dma_generic_free_coherent",
      "external": true,
      "loc": "arch/x86/kernel/pci-dma.c:123",
      "file": "arch/x86/kernel/pci-dma.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "arch/x86/kernel/pci-swiotlb.c:x86_swiotlb_free_coherent",
        "arch/x86/kernel/amd_gart_64.c:gart_free_coherent"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071579049504,
      "name": "dma_generic_free_coherent",
      "section": ".text",
      "bind": "STB_GLOBAL",
      "size": 39
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>4.15.0-20-generic-amd64</code>: ✅</summary>

```c
void dma_generic_free_coherent(struct device * dev, size_t size, void * vaddr, dma_addr_t dma_addr, long unsigned int attrs)
```

```json
{
  "name": "dma_generic_free_coherent",
  "collision_type": "Unique Global",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071579058512,
      "name": "dma_generic_free_coherent",
      "external": true,
      "loc": "arch/x86/kernel/pci-dma.c:127",
      "file": "arch/x86/kernel/pci-dma.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "arch/x86/kernel/pci-swiotlb.c:x86_swiotlb_free_coherent",
        "arch/x86/kernel/amd_gart_64.c:gart_free_coherent"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071579058512,
      "name": "dma_generic_free_coherent",
      "section": ".text",
      "bind": "STB_GLOBAL",
      "size": 39
    }
  ]
}
```
</details>
</li>
<li>
In <code>4.18.0-10-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.0.0-13-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.3.0-18-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.8.0-25-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.11.0-16-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.13.0-19-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.15.0-25-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.19.0-21-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>6.2.0-20-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>6.5.0-9-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>6.8.0-31-generic-amd64</code>: Absent ❓
</li>
</ul>
<b>Arch</b>
<ul>
<li>
In <code>5.4.0-26-generic-arm64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-generic-armhf</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-generic-ppc64el</code>: Absent ❓
</li>
<li>
In <code>5.4.0-24-generic-riscv64</code>: Absent ❓
</li>
</ul>
<b>Flavor</b>
<ul>
<li>
In <code>5.4.0-1009-aws-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-1010-azure-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-1009-gcp-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-lowlatency-amd64</code>: Absent ❓
</li>
</ul>

## Differences
<b>Regular</b>
<ul>
<li>
<details>
<summary>Changed between <code>4.4.0-21-generic-amd64</code> and <code>4.8.0-22-generic-amd64</code> ❓</summary>
<ul>
<li>
<b>Param type changed. </b>
<code>struct dma_attrs * attrs</code> ➡️ <code>long unsigned int attrs</code>
</li>
</ul>
</details>
</li>
<li>
No changes between <code>4.8.0-22-generic-amd64</code> and <code>4.10.0-19-generic-amd64</code> ✅
</li>
<li>
No changes between <code>4.10.0-19-generic-amd64</code> and <code>4.13.0-16-generic-amd64</code> ✅
</li>
<li>
No changes between <code>4.13.0-16-generic-amd64</code> and <code>4.15.0-20-generic-amd64</code> ✅
</li>
<li>
<details>
<summary>Removed between <code>4.15.0-20-generic-amd64</code> and <code>4.18.0-10-generic-amd64</code> ➖</summary>

```c
void dma_generic_free_coherent(struct device * dev, size_t size, void * vaddr, dma_addr_t dma_addr, long unsigned int attrs)
```
</details>
</li>
</ul>
