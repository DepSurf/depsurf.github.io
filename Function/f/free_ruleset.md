# Function: <code>free_ruleset</code>

## Status
<b>Regular</b>
<ul>
<li>
In <code>4.4.0-21-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>4.8.0-22-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>4.10.0-19-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>4.13.0-16-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>4.15.0-20-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>4.18.0-10-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.0.0-13-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.3.0-18-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.8.0-25-generic-amd64</code>: Absent ❓
</li>
<li>
In <code>5.11.0-16-generic-amd64</code>: Absent ❓
</li>
<li>
<details>
<summary>In <code>5.13.0-19-generic-amd64</code>: ✅</summary>

```c
void free_ruleset(const struct landlock_ruleset * ruleset)
```

```json
{
  "name": "free_ruleset",
  "collision_type": "Unique Static",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071584315856,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/landlock/ruleset.c:356",
      "file": "security/landlock/ruleset.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "security/landlock/ruleset.c:landlock_merge_ruleset",
        "security/landlock/ruleset.c:free_ruleset_work"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071584315856,
      "name": "free_ruleset",
      "section": ".text",
      "bind": "STB_LOCAL",
      "size": 215
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>5.15.0-25-generic-amd64</code>: ✅</summary>

```c
void free_ruleset(const struct landlock_ruleset * ruleset)
```

```json
{
  "name": "free_ruleset",
  "collision_type": "Unique Static",
  "inline_type": "No",
  "funcs": [
    {
      "addr": 18446744071584702928,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/landlock/ruleset.c:356",
      "file": "security/landlock/ruleset.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "security/landlock/ruleset.c:landlock_merge_ruleset",
        "security/landlock/ruleset.c:free_ruleset_work"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071584702928,
      "name": "free_ruleset",
      "section": ".text",
      "bind": "STB_LOCAL",
      "size": 215
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>5.19.0-21-generic-amd64</code>: Collision, Selective Inline ❓</summary>

```c
void free_ruleset(struct aa_ruleset * rules)
```

```json
{
  "name": "free_ruleset",
  "collision_type": "Static-Static Collision",
  "inline_type": "Selective",
  "funcs": [
    {
      "addr": 18446744071585244498,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/apparmor/policy.c:207",
      "file": "security/apparmor/policy.c",
      "inline": "not declared, inlined",
      "caller_inline": [
        "security/apparmor/policy.c:aa_free_profile"
      ],
      "caller_func": []
    },
    {
      "addr": 18446744071585367408,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/landlock/ruleset.c:359",
      "file": "security/landlock/ruleset.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "security/landlock/ruleset.c:landlock_merge_ruleset",
        "security/landlock/ruleset.c:free_ruleset_work"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071585367408,
      "name": "free_ruleset",
      "section": ".text",
      "bind": "STB_LOCAL",
      "size": 201
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>6.2.0-20-generic-amd64</code>: Collision, Selective Inline ❓</summary>

```c
void free_ruleset(struct aa_ruleset * rules)
```

```json
{
  "name": "free_ruleset",
  "collision_type": "Static-Static Collision",
  "inline_type": "Selective",
  "funcs": [
    {
      "addr": 18446744071585976252,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/apparmor/policy.c:209",
      "file": "security/apparmor/policy.c",
      "inline": "not declared, inlined",
      "caller_inline": [],
      "caller_func": []
    },
    {
      "addr": 18446744071586118128,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/landlock/ruleset.c:359",
      "file": "security/landlock/ruleset.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "security/landlock/ruleset.c:landlock_merge_ruleset",
        "security/landlock/ruleset.c:free_ruleset_work"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071586118128,
      "name": "free_ruleset",
      "section": ".text",
      "bind": "STB_LOCAL",
      "size": 201
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>6.5.0-9-generic-amd64</code>: Collision, Selective Inline ❓</summary>

```c
void free_ruleset(struct aa_ruleset * rules)
```

```json
{
  "name": "free_ruleset",
  "collision_type": "Static-Static Collision",
  "inline_type": "Selective",
  "funcs": [
    {
      "addr": 18446744071586209158,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/apparmor/policy.c:246",
      "file": "security/apparmor/policy.c",
      "inline": "not declared, inlined",
      "caller_inline": [],
      "caller_func": []
    },
    {
      "addr": 18446744071586356896,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/landlock/ruleset.c:359",
      "file": "security/landlock/ruleset.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "security/landlock/ruleset.c:landlock_merge_ruleset",
        "security/landlock/ruleset.c:free_ruleset_work"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071586356896,
      "name": "free_ruleset",
      "section": ".text",
      "bind": "STB_LOCAL",
      "size": 201
    }
  ]
}
```
</details>
</li>
<li>
<details>
<summary>In <code>6.8.0-31-generic-amd64</code>: Collision, Selective Inline ❓</summary>

```c
void free_ruleset(struct aa_ruleset * rules)
```

```json
{
  "name": "free_ruleset",
  "collision_type": "Static-Static Collision",
  "inline_type": "Selective",
  "funcs": [
    {
      "addr": 18446744071586461318,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/apparmor/policy.c:247",
      "file": "security/apparmor/policy.c",
      "inline": "not declared, inlined",
      "caller_inline": [],
      "caller_func": []
    },
    {
      "addr": 18446744071586624112,
      "name": "free_ruleset",
      "external": false,
      "loc": "security/landlock/ruleset.c:486",
      "file": "security/landlock/ruleset.c",
      "inline": "seen, unknown",
      "caller_inline": [],
      "caller_func": [
        "security/landlock/ruleset.c:landlock_merge_ruleset",
        "security/landlock/ruleset.c:free_ruleset_work"
      ]
    }
  ],
  "symbols": [
    {
      "addr": 18446744071586624112,
      "name": "free_ruleset",
      "section": ".text",
      "bind": "STB_LOCAL",
      "size": 251
    }
  ]
}
```
</details>
</li>
</ul>
<b>Arch</b>
<ul>
<li>
In <code>5.4.0-26-generic-arm64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-generic-armhf</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-generic-ppc64el</code>: Absent ❓
</li>
<li>
In <code>5.4.0-24-generic-riscv64</code>: Absent ❓
</li>
</ul>
<b>Flavor</b>
<ul>
<li>
In <code>5.4.0-1009-aws-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-1010-azure-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-1009-gcp-amd64</code>: Absent ❓
</li>
<li>
In <code>5.4.0-26-lowlatency-amd64</code>: Absent ❓
</li>
</ul>

## Differences
<b>Regular</b>
<ul>
<li>
<details>
<summary>Added between <code>5.11.0-16-generic-amd64</code> and <code>5.13.0-19-generic-amd64</code> ➕</summary>

```c
void free_ruleset(const struct landlock_ruleset * ruleset)
```
</details>
</li>
<li>
No changes between <code>5.13.0-19-generic-amd64</code> and <code>5.15.0-25-generic-amd64</code> ✅
</li>
<li>
<details>
<summary>Changed between <code>5.15.0-25-generic-amd64</code> and <code>5.19.0-21-generic-amd64</code> ❓</summary>
<ul>
<li>
<b>Param added. </b>
<code>struct aa_ruleset * rules</code>
</li>
<li>
<b>Param removed. </b>
<code>const struct landlock_ruleset * ruleset</code>
</li>
</ul>
</details>
</li>
<li>
No changes between <code>5.19.0-21-generic-amd64</code> and <code>6.2.0-20-generic-amd64</code> ✅
</li>
<li>
No changes between <code>6.2.0-20-generic-amd64</code> and <code>6.5.0-9-generic-amd64</code> ✅
</li>
<li>
No changes between <code>6.5.0-9-generic-amd64</code> and <code>6.8.0-31-generic-amd64</code> ✅
</li>
</ul>
